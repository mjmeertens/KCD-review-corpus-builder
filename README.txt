...
Because of the large amounts of data, I have cached the intermediate steps so you can safely run the code without your computer crashing. These are in the 'cache' folder. However, ironically, some are still to big for GitHub so you will have to run them yourself. Make sure you're running the script from the right interpreter and the right working directory so it finds the .pkl files. If not, it will still run the heavy parts.
